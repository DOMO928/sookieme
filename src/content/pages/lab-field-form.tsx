    ...
    <>
      <section className="lab-head">
        ...
        <p className="lab-description">
          {t('labFieldForm.description')}
          <br />
          {t('labFieldForm.subtitle')}
        </p>
        ...
        <div className="lab-console">
          ...
          <div className="lab-readouts">
            ...
            <p id="measurement-result" className="lab-note" aria-live="polite">
              {t('labFieldForm.measurementNote')}
            </p>
          </div>
        </div>
      ...
      <article className="story" id="implementation">
        <section className="story-lead">
          <p className="eyebrow">{t('labFieldForm.category')}</p>
          <h2>{t('labFieldForm.persistentScene')}</h2>
          <p>{t('labFieldForm.overview')}</p>
          <p>{t('labFieldForm.visualReferences')}</p>
        </section>
        <section>
          <p className="eyebrow">{t('labFieldForm.dataCompute')}</p>
          <h2>Surface Morphing</h2>
          <p>{t('labFieldForm.sampleIdentity')}</p>
          <p>{t('labFieldForm.simulation')}</p>
          <p>{t('labFieldForm.driftAndDeformation')}</p>
          <p>{t('labFieldForm.pointerAndScroll')}</p>
          <div className="technical-flow">
            <div className="flow-origin">
              <code>Position + velocity</code>
              <span>{t('labFieldForm.bufferCaption')}</span>
            </div>
          ...
          <p>{t('labFieldForm.bufferOwnership')}</p>
        </section>
        <section>
          <p className="eyebrow">RENDERING & LIFETIME</p>
          <h2>Rendering & Lifetime</h2>
          <p>{t('labFieldForm.particleAppearance')}</p>
          <p>{t('labFieldForm.navigationLifecycle')}</p>
          <p>{t('labFieldForm.fallbackBehavior')}</p>
        </section>
        <section>
          <p className="eyebrow">{t('labFieldForm.sourceReproduction')}</p>
          <h2>{t('labFieldForm.sourceCode')}</h2>
          <p>{t('labFieldForm.sourceOverview')}</p>
          <div className="archive-list">
            <LocalLink href="/source/field.wgsl.txt" target="_blank" locale={locale}>
              ...
              <span>
                ...
                <small>{t('labFieldForm.shaderSourceLabel')}</small>
              </span>
            ...
            <LocalLink href="/source/lib.rs.txt" target="_blank" locale={locale}>
              ...
              <span>
                <b>Rust / wgpu</b>
                <small>{t('labFieldForm.rustSourceLabel')}</small>
              </span>
              <Icon name="arrow-up-right" />
            </LocalLink>
            <LocalLink href="/source/controller.ts.txt" target="_blank" locale={locale}>
              ...
              <span>
                ...
                <small>{t('labFieldForm.controllerSourceLabel')}</small>
              </span>
              <Icon name="arrow-up-right" />
            </LocalLink>
            <LocalLink href="/source/field-form-source.zip" download locale={locale}>
              ...
              <span>
                <b>{t('labFieldForm.sourceDownload')}</b>
                <small>{t('labFieldForm.archiveDescription')}</small>
              </span>
